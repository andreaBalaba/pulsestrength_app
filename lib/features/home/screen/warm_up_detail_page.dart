...
class WarmUpDetailPage extends StatefulWidget {
  final dynamic data;
  final List warmups;
  final int currentIndex;

  const WarmUpDetailPage({
    super.key, 
    required this.data, 
  ...
  @override
  State<WarmUpDetailPage> createState() => _WarmUpDetailPageState();
}

class _WarmUpDetailPageState extends State<WarmUpDetailPage> {
  bool isDone = false;
  Timer? _timer;
  ...
  void _goToNextWarmUp() {
    _audioPlayer.stop();
    // Go back to the warm-up list with the status
  ...
  void _handleButtonPress() {
    ...
    if (isDone) {
      // When Finish is clicked after timer completes
      _goToNextWarmUp();
      return;
    }
      ...
      appBar: AppBar(
        ...
        actions: [
          TextButton(
            onPressed: () {
              setState(() {
                isDone = false; // Ensure it's marked as skipped
              });
              _goToNextWarmUp();
            },
